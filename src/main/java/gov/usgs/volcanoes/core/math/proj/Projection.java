...
abstract public class Projection {
    ...
    public static double distanceBetween(Point2D.Double pt1, Point2D.Double pt2) {
        if (pt1 == null || pt2 == null || Double.isNaN(pt1.x) || Double.isNaN(pt1.y) || Double.isNaN(pt2.x)
                || Double.isNaN(pt2.y))
            return Double.NaN;
    ...
    public static Point2D.Double getPointFrom(Point2D.Double origin, double c, double az) {
        ...
        pt.y = Math.asin(Math.sin(phi1) * Math.cos(c) + Math.cos(phi1) * Math.sin(c) * Math.cos(az)) * RAD2DEG;
    ...
    public void mapRect(FastProjector fast, double x1, double y1, double x2, double y2, int dx, int dy, int dw, int dh,
            int[] src, int[] dest, int sw, int sh, int scan, GeoRange range) {
    ...
    public void fastMapRect(FastProjector fast, double x1, double y1, double x2, double y2, int dx, int dy, int dw,
            int dh, int[] src, int[] dest, int sw, int sh, int scan, GeoRange range) {
    ...
    public BufferedImage getProjectedImage(int n, int dw, int dh, BufferedImage src, GeoRange gr, double mw, double me,
            double ms, double mn) {
    ...
    public BufferedImage getProjectedImage(int n, int dw, int dh, int[] src, int sw, int sh, GeoRange gr, double mw,
            double me, double ms, double mn) {
        ...
        for (int i = -n; i < n; i++)
            for (int j = -n; j < n; j++) {
                ...
                fastMapRect(fast, x0, y0, x1, y1, (n + i) * stepX, (n - j - 1) * stepY, stepX, stepY, src, buf, sw, sh,
                        dw, gr);
            }
